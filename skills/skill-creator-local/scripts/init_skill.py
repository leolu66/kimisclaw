...
def load_external_guide():
    """Load guide from SKILL_DO.md if available."""
    try:
        # Try to find SKILL_DO.md in workspace
        workspace_root = Path(__file__).parent.parent.parent.parent
        skill_do_path = workspace_root / "skills" / "SKILL_DO.md"
        
        if skill_do_path.exists():
            content = skill_do_path.read_text(encoding="utf-8")
            # Extract file path section
            if "文件路径规范" in content:
                start = content.find("文件路径规范")
                end = content.find("**文件要求：**", start)
                if end == -1:
                    end = content.find("## 3️", start)
                if start != -1 and end != -1:
                    return "\n## 开发规范参考\n\n" + content[start:end].strip()
    except Exception:
        pass
    
    # Return built-in guide if external file not found
...
def init_skill(skill_name, path, resources, include_examples):
    ...
    # Append development guide
    guide_content = load_external_guide()
